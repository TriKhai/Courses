...
void BranchAndBound(Graph *G, int i, int current, float p_cost, float *final_cost, Edge x[]){    
    int n = G->n;
        ...
        if(current != next && !G->E[current][next].mark && !isCycle(x, i, next)){
            ...
            if(lbound < *final_cost){
                ...
                if(i == n-2)
                    ...
                    BranchAndBound(G, i+1, next, cost, final_cost, x);
            }
...
int main(){
    ...
    while(1){
        ...
        BranchAndBound(&G, 0, s-97, 0, &final_cost, x);
