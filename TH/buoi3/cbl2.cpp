...
void ReadFile(Balo *Bl) {
	...
	int i = 0;
	while(!feof(f)) {
		fscanf(f, "%d%d%d%[^\n]", &Bl->Items[i].w, &Bl->Items[i].v,
		&Bl->Items[i].gh, &Bl->Items[i].name);
...
void printInput(Balo *Bl) {
	...
	printf("\n[%d-%d]\n", Bl->n, Bl->W);
	printf("+----+----+----+-------------------+-----+\n");
	printf("| gi | vi | gh |       name        | dg  |\n");
	printf("|----+----+----+-------------------+-----|\n");
	while(i < n) {
		printf("|%3d |%3d |%3d |%-18s |%3.2f |\n", Bl->Items[i].w, 
		Bl->Items[i].v, Bl->Items[i].gh,  Bl->Items[i].name, Bl->Items[i].dg);
		i++;
	}	
	printf("+----+----+----+-------------------+-----+\n");
}
...
void NhanhCan(Balo *Bl, int x[], int pTGT, int i, float *GTLNTT, int W) {
	int n = Bl->n;
	for(int j=min(Bl->Items[i].gh, W/Bl->Items[i].w); j>=0; j--) {
...
void printChart (Balo *B){
	...
	for(int i = 0, k = 1; i < n; i++){
		...
		B->Items[i].w , B->Items[i].v, B->Items[i].gh, B->Items[i].dg, B->Items[i].sl);
